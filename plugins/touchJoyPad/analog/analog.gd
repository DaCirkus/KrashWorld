...
func need2ChangeActivePointer(event) -> bool: #touch down inside analog	
	if event is InputEventScreenTouch:
		var gpos = global_position
		if (gpos - event.position).length() > input_radius:
			return false
...
func map_analog_dpad():
	Input.action_press("character_left") if currentForce.x < -0.2 else Input.action_release("character_left")
	Input.action_press("character_right") if currentForce.x > 0.2 else Input.action_release("character_right")
	Input.action_press("character_backward") if currentForce.y < -0.2 else Input.action_release("character_backward")
	Input.action_press("character_forward") if currentForce.y > 0.2 else Input.action_release("character_forward")
